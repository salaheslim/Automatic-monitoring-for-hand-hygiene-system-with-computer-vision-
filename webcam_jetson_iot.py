...
MQTT_PORT = 1883
MQTT_TOPIC = "cardiffmet/handhygiene/jetson/session"
MQTT_TOPIC_STEP = "cardiffmet/handhygiene/jetson/step"
MQTT_TOPIC_STATUS = "cardiffmet/handhygiene/jetson/status"
...
while True:
        ...
        if smooth_pred == current_step_idx and \
           vote_conf >= VOTE_THRESHOLD and hands_detected > 0:
            ...
            if time.time() - confirm_start >= CONFIRMATION_TIME:
                ...
                # Publish step
                mqtt_client.publish(MQTT_TOPIC_STEP, json.dumps({
                    "session_id": session_id,
                    "device": "Jetson_Nano_4GB",
                    "step_number": current_step_idx + 1,
                    "step_name": STEPS[current_step_idx],
                ...
                if current_step_idx >= 7:
                    ...
                    avg_time = float(np.mean([r['scan_time'] for r in step_results]))
                    # Publish session
                    mqtt_client.publish(MQTT_TOPIC, json.dumps({
                        "session_id": session_id,
                        "device": "Jetson_Nano_4GB",
                        "camera": "Webcam_RGB_GPU",
                        "steps_completed": 7,
                        "total_steps": 7,
                        "compliance": 100,
                    ...
                    print("Session Complete — Published to IoT")
        else:
            confirm_start = None
    ...
    # MQTT indicator
    mqtt_color = GREEN if mqtt_client.is_connected() else RED
    cv2.circle(canvas, (995,15), 8, mqtt_color, -1)
    cv2.putText(canvas, "MQTT", (960,35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.35, mqtt_color, 1)

    cv2.putText(canvas, "Hand Hygiene Monitor | IoT",
                (648,25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, TEAL, 2)
    cv2.putText(canvas, f"FPS:{fps:.1f} | Hands:{hands_detected} | Norm:ON",
    ...
    cv2.putText(canvas,
                f"Avg:{avg_time:.1f}s  Total:{total_time:.1f}s  FPS:{fps:.1f}",
                (648,445), cv2.FONT_HERSHEY_SIMPLEX, 0.45, WHITE, 1)
    cv2.putText(canvas, f"IoT: {MQTT_BROKER}",
                (648,465), cv2.FONT_HERSHEY_SIMPLEX, 0.38, TEAL, 1)

    if session_complete:
        cv2.rectangle(canvas, (644,0), (1015,480), GREEN, 3)
        cv2.putText(canvas, "PUBLISHED TO IOT DASHBOARD",
                   (648,478), cv2.FONT_HERSHEY_SIMPLEX, 0.5, GREEN, 2)

    cv2.imshow("Hand Hygiene Monitor | Jetson Nano IoT", canvas)
