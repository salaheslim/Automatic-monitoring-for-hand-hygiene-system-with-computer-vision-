...
os.makedirs('jetson_data', exist_ok=True)
...
# Frames to collect per step - more = better accuracy
FRAMES_PER_STEP = 300

print("="*50)
print("JETSON DATA COLLECTION")
print("="*50)
print(f"Collecting {FRAMES_PER_STEP} frames per step")
print("Click on camera window first!")
print("Press 0-6 to start recording that step")
...
while True:
    ...
    if current_step is not None and hands_found and recording:
        ...
        if count >= FRAMES_PER_STEP:
            filename = f"jetson_data/step{current_step}_jetson.npy"
        ...
        cv2.putText(frame, "Show hand to start recording",
    ...
    for s in range(7):
        ...
        if count >= FRAMES_PER_STEP:
            ...
            color = (0,165,255)
            status = f"{count}/{FRAMES_PER_STEP}"
        cv2.putText(frame, f"S{s+1}: {status}",
                   (480, 60+s*30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)

    # Hand detection indicator
    hand_color = (0,255,0) if hands_found else (0,0,255)
    hand_text = f"Hands: {len(results.multi_hand_landmarks) if results.multi_hand_landmarks else 0}"
    cv2.putText(frame, hand_text,
               (10, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.6, hand_color, 2)

    cv2.imshow("Jetson Data Collection", frame)
...
for s in range(7):
    count = len(step_data[s])
    total += count
    status = "✅ DONE" if count >= FRAMES_PER_STEP else f"❌ {count} frames only"
